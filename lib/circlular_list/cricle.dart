...
class _DragModel {
  ...
  double getAngleDiff(PolarCoord updatePolar,DragAngleRange? dragAngleRange) {
    ...
    angleDiff = limitAngle(angleDiff);
    return angleDiff;
  }

  double limitAngle(double angleDiff) {
    final double partAngle = 2 * pi / totalParts;

    // Check if the angle crosses more than 50% of its portion of 12 parts
    if (angleDiff > 0) {
      int partsCrossed = (angleDiff / partAngle).ceil();
      if (partsCrossed >= totalParts / 2) {
        angleDiff = (totalParts - partsCrossed) * partAngle;
      }
    } else {
      int partsCrossed = (-angleDiff / partAngle).ceil();
      if (partsCrossed >= totalParts / 2) {
...
//   List<String> num1 = [];
//   List<String> originalList = []; // Store the original list
//   int tappedIndex = -1;
//
//   @override
//   void initState() {
//     num1 = Numbers.num;
//     originalList = List<String>.from(num1); // Save a copy of the original list
//     super.initState();
//   }
//
//   @override
...
//   }
//
//   List<String> rotateList(List<String> list, int index) {
//     return List.from(list.sublist(index)..addAll(list.sublist(0, index)));
//   }
//
// }
